...
class pokebot extends start
{
  ...
  protected $function_name = "Poke Bot";
	...
	private function usersOnChannel()
  {
		...
		foreach($clientList as $clientListTemp)
    {
			...
			if(($clientListTemp['cid'] == $this->config['channel_id']) && ($clientListTemp['client_type'] == 0))
      {
  ...
  */
	private function channelStatus()
  {
		...
		foreach($channelList as $channelListTemp)
    {
			...
			if($channelListTemp['cid'] == $this->config['channel_id'])
      {
				...
				if($this->timeOfflineStatus > 0)
        {
					...
					$this->poketimer = date('r', time() + $this->config['time_offline']);
  ...
  */
	private function poke()
  {
    $tsAdmin = $this->multibotCore->gettsAdmin();
		...
		$adminGroups = explode(',', $this->config['group']);
		...
		if($adminList)
    {
			...
			foreach($adminList as $adminListTemp)
      {
				...
				$tsAdmin->clientPoke($adminListTemp['clid'], $this->config['poke_message']);
			...
			$this->poketimer = date('r', time() + $this->config['time']);
			#Zwraca wartość true
			return true;
		}
    ...
    if($channelStatus == true)
    {
      ...
      if($pokeStatus == true)
      {
        ...
        foreach($diff as $diffTemp)
        {
          ...
          $tsAdmin->sendMessage(1, $diffTemp, $this->config['Admin_online']);
        }
      }
      #Jeżeli wartość funkcji poke() = false
        ...
        foreach($diff as $diffTemp)
        {
          ...
          $tsAdmin->sendMessage(1, $diffTemp, $this->config['Admin_offline']);
        }
      }
#sadasd
