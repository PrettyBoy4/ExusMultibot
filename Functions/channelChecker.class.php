...
//*******************************************************************************************
  ...
  private $config;
  protected $functionName = "channelChecker";
...
function start_function() {
  ...
  foreach($config as $id => $section) {
    if(is_int($id))  {
      ...
      $channelStats = parse_ini_file("Configs/Functions/channelChecker.ch", true);
      ...
      foreach($channelList as $channel)  {
        ...
        if($channel['cid'] == $section[1])  {
          ...
          if(($channelStats[$channel['cid']]['time_to_write'] <= date("d.m.y,H:i:s")) || ($channelStats[$channel['cid']]['write'] == "2"))  {
            ...
            $sourceMessage = file_get_contents("Configs/Functions/" . $config['message']);
        ...
        $this->writeInfo("Configs/Functions/channelChecker.ch", $channelStats);
      }
    }
  }
}
...
function __construct(multibotCore $multibotCore)  {
  ...
  $this->config = $multibotCore->config['multibotConfig']['channelChecker'];
}
