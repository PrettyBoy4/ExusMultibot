...
protected $function_name = "Admin Channel Status";
//*******************************************************************************************
//************************************ Public Functions *************************************
//******************************************************************************************
public function start_function() {
  ...
  $config = $config['adminchannelstatus']['adminchannelstatus'];
...
private function setChannel($chid, $channelInfo, $status, $busy = false) {
  ...
  if(!$permList['success']) {
  	$client_name = $tsAdmin->clientGetNameFromUid($config[$chid]);
  	if(!$client_name['success']) {
  		$this->multibotCore->AddError("Użtkownik o uid: " . $config[$chid] . " nie istnieje;");
  	}else{
  		$this->multibotCore->addError("Kanał użytkownika " . $client_name['data']['name'] . " nie istnieje");
  	}
  	return false;
  }
